...
> **Note on Kimi Code CLI:** its headless CLI has no scoped read-only mode either, and it's worse than Antigravity's/Cline's — `--plan` cannot even be *passed* alongside the one-shot prompt flag (verified live: the CLI rejects the combination outright), so text-mode (plan/review) runs use no permission flag at all. Verified directly that a bare invocation with zero flags still wrote a file and ran a shell command with no approval prompt. Edit-mode runs use `--yolo` explicitly, though it changes nothing observable. Also note: its CLI only accepts the prompt as a command-line argument (no stdin, no file-path option), so it must be installed via the **official installer**, not npm — an npm install produces a shell-shim wrapper this integration can't safely drive, and very large context packs can fail outright with a "prompt too large" error rather than a silent truncation. Commit or back up before using it, or pick another provider.
