/**
 * Chat interaction transaction record (Part 2 port of
 * `src/types/chatInteractionTransactionV1.ts`; original commentary retained,
 * with Node Buffer/crypto replaced by portable TextEncoder + sha256V1).
 *
...
import { sha256HexUtf8V1, utf8ByteLengthV1 } from "./sha256V1";
import { decodeTaskActionOutcomeV1, outcomeCorrelationV1, TaskActionOutcomeV1 } from "./taskActionOutcomeV1";
import { STAGE_ORDER, TaskStage } from "./taskProgressV1";
import {
...
const sha256HexUtf8 = sha256HexUtf8V1;
...
export function encodeChatInteractionTransactionV1(
  ...
  return new TextEncoder().encode(canonicalJsonTextV1(transaction));
}
...
function decodeInputSnapshot(raw: unknown): ChatTransactionInputSnapshotV1 | string {
  ...
  if (utf8ByteLengthV1(raw.canonicalJson) > MAX_INPUT_SNAPSHOT_CANONICAL_BYTES_V1) {
