/**
 * AI result-contract prompt fragment (Part 2 port of
 * `src/prompts/aiResultContractV1.ts` — the `ensemble.aiResultContract.v1`
...
/** The frame markers the contract requires around the single JSON result line. */
export const AI_RESULT_FRAME_OPEN_V1 = "<<<ENSEMBLE_AI_RESULT_V1>>>";
export const AI_RESULT_FRAME_CLOSE_V1 = "<<<END_ENSEMBLE_AI_RESULT_V1>>>";
...
/**
 * The closed set of completed-content type names, verbatim from the
 * extension's `CompletedContentV1["contentType"]`
 * (`src/types/aiResultEnvelope.ts`). Declared as a literal union because
 * this package ports the frame contract, not the whole envelope parser; the
 * shape-hint table below pins the roster.
 */
export type CompletedContentTypeNameV1 =
