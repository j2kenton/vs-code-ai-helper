/**
 * Closed coordinator outcome union (Part 2 port of
 * `src/types/taskActionOutcomeV1.ts`).
 *
...
/**
 * Malformed-result codes, verbatim from the extension's envelope parser
 * (`src/types/aiResultEnvelope.ts` — `MalformedAiResultV1["code"]`). Declared
 * as a literal union here because this package ports only the closed outcome
 * contract, not the whole envelope parser; the MALFORMED_RESULT_CODES_V1 set
 * below pins the roster.
 */
export type MalformedResultCodeV1 =
  | "invalidFrame"
  | "invalidJson"
  | "invalidEnvelope"
...
/**
 * Workflow-unavailable codes, verbatim from
 * `src/types/workflowAvailabilityV1.ts`.
 */
