/**
 * Per-stage fallback state (Part 2 port of `src/types/fallbackStateV1.ts`).
 *
 * Scalar boolean input is INVALID — a historical shape that stored one
 * boolean for the whole task cannot be mapped onto per-stage state without
 * guessing, so the strict decoder rejects it into recovery instead of
 * coercing.
 */
