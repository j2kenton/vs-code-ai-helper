 ...
 * content-driven rejection — `decodeInputSnapshot` (below) producing exactly
 * "inputSnapshot exceeds the N-byte canonical limit" — as opposed to some
 * other rejection (a malformed correlation tuple, an undecodable record).
 *
 * Item 9 ("make the stage chat a record of work", Part 16 step 43): an
 * oversized input snapshot cannot decode differently on retry with an
 * unchanged prompt, so reporting it `retryable: true` (the blanket default
 * every `chatTransactionRejected` used to get) turns a structural limit into
 * a loop the user runs by hand — a real review stalled for days on exactly
 * this. Matched on the reason text rather than a separate error code because
 * `decodeInputSnapshot` has no code of its own to plumb through; the message
 * is generated by our own store from its own record contract (see
 * `chatTransactionFailureCodeV1`'s doc comment) and is stable enough to
 * pattern-match.
 */
export function isInputSnapshotSizeRejectionReasonV1(reason: string): boolean {
  return reason.includes("exceeds the") && reason.includes("byte canonical limit");
}
