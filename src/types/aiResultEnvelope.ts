/**
 * Standard machine-readable envelope for AI action output (plan §3.5).
 *
 * Every AI stage action should be able to resolve to exactly one of these
 * outcomes so the product can distinguish completed output, structured
 * questions, user cancellation, failure, and malformed/unparseable output
 * without each call site inventing its own ad-hoc checks. Every envelope
 * carries a full correlation tuple (actionCorrelationV1.ts) so a
...
export interface CommitMetadataCompletedV1 {
  ...
  readonly schemaVersion: 1;
  /** Conventional-Commits subject line, <=72 chars, matching commitAndPushTask.ts's contract. */
...
// grammar) naturally rejects comments, trailing commas, and bare NaN/Infinity
// that JSON.parse's more permissive V8 implementation might otherwise let
// slip in edge cases.
// ---------------------------------------------------------------------------
...
function parseStrictJsonV1(text: string): StrictJsonParseResult {
  ...
  function parseObject(depth: number): Record<string, unknown> {
    ...
    // (plain `{}` object-literal semantics would let it do the latter).
...
function decodePreflightOperation(
  ...
  if (!isWrite) {
    ...
    return { stepId, kind, rootId: raw.rootId, relativePath: raw.relativePath, targetObservationId: raw.targetObservationId, parentChain };
  }
  ...
  const unknownField = rejectUnknownFields(
    ...
    new Set([
      ...
      "parentChain", "contentBase64", "decodedByteLength", "contentSha256",
    ]),
    `operation ${stepId}`
  );
  if (unknownField) {
...
function decodePreflightPlan(raw: Record<string, unknown>): ContentDecodeResult {
  ...
  if (raw.operations.length > MAX_PREFLIGHT_OPERATIONS_V1) {
    return { ok: false, reason: `preflight-plan.v1 exceeds ${MAX_PREFLIGHT_OPERATIONS_V1} operations` };
  }
  ...
  // Parent-chain referential integrity (plan §7.3): a "createdByStep" link
  // must reference a strictly earlier createDirectory step in this same
  // operation list.
...
function decodeEditExecution(raw: Record<string, unknown>): ContentDecodeResult {
  ...
  if (!Array.isArray(raw.receiptIds) || raw.receiptIds.some((r) => typeof r !== "string" || r.length === 0)) {
...
function decodeCompletedContentV1(raw: unknown): ContentDecodeResult {
  ...
  if (raw.schemaVersion !== 1) {
    return { ok: false, reason: `unsupported content "schemaVersion": ${JSON.stringify(raw.schemaVersion)}` };
  }
...
const ENVELOPE_TOP_LEVEL_FIELDS_V1 = new Set([
  "version", "correlation", "kind", "content", "questions", "reason", "code", "message", "retryable",
]);
...
function decodeEnvelopeV1(
  ...
  if (value.version !== 1) {
    return malformed("invalidEnvelope", raw, `unsupported envelope "version": ${JSON.stringify(value.version)}`);
  }
  ...
  // JSON parser's Object.create(null) result) so downstream consumers —
  // including strict deep-equality in tests — see an ordinary object shape.
  ...
  switch (value.kind) {
    ...
    case "failed": {
      ...
      if (typeof value.message !== "string" || utf8ByteLength(value.message) > MAX_FAILURE_MESSAGE_BYTES_V1) {
      ...
      return { version: 1, correlation, kind: "failed", code: value.code, message: value.message, retryable: value.retryable };
    }
    default:
      return malformed("invalidEnvelope", raw, `unrecognized envelope "kind": ${JSON.stringify(value.kind)}`);
  }
}
 ...
 * stale-attempt result (resultCorrelationMismatch) before any content is
...
export function parseAiResultEnvelopeV1(
  ...
  if (jsonLine.includes("\n") || jsonLine.includes("\r")) {
    return malformed("invalidFrame", raw, "the JSON payload must be a single line (no embedded line breaks)");
  }
