...
async function performStageSwap(node: StageNode | undefined, kind: StageSwapKind): Promise<void> {
  ...
  await runTrackedOperation(
    ...
    async () => {
      ...
      // Revalidate the redo sidecar against the just-read bytes, in the same
      // locked section (runTrackedOperation) immediately before swapping. If
      // the sidecar exists but its fingerprints no longer match the files —
      // e.g. an out-of-band edit, or a partially-applied earlier swap — the
      // sidecar's picture of "which side the artifact is on" can no longer be
      // trusted, so refuse the swap rather than risk overwriting content the
      // sidecar never saw. A missing/unknown sidecar is the safe default and
      // is not checked (matches pre-existing behavior).
      ...
      if (sidecar) {
        ...
        // Direction re-check under the lock: even with matching fingerprints,
        // a concurrent swap serialized just ahead of this one (same task, same
        // exclusive operation lock) could have flipped the direction. Refuse
        // rather than perform the wrong-labeled swap.
      ...
      // Journal-backed SWAP: the artifact takes the backup's content and the
      // backup takes the pre-swap content, so both revert and redo are
      ...
      const writeArtifact = async (content: Uint8Array): Promise<void> => {
        if (doc) {
          ...
          if (!(await doc.save())) {
            ...
            if (!rolledBack) {
              // Buffer mutated, disk not, and the rollback edit was refused:
              // the journal must survive so activation-time recovery
              // finishes the swap.
