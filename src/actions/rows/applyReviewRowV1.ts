/**
 * `applyReview.v1` registry row (plan §6.5, "Migrate remaining text actions").
 *
 * Target artifact: task artifact updated by text apply review (e.g. `<task-folder>/plan.md`).
...
export interface ApplyReviewActionInputV1 {
...
export function validateApplyReviewInputV1(
  rawInput: unknown
): TaskActionInputValidationResultV1 {
  ...
  const validated: ApplyReviewActionInputV1 = {
    prompt: raw.prompt,
    targetLocator: {
...
class ApplyReviewPromotionErrorV1 extends Error {
  constructor(message: string) {
    super(message);
    this.name = "ApplyReviewPromotionErrorV1";
  }
}

async function promoteApplyReviewContentV1(
  content: CompletedContentV1,
  context: TaskActionExecutionContextV1
): Promise<TaskActionPromotionCodeV1> {
  if (!isMarkdownArtifactV1(content)) {
    throw new ApplyReviewPromotionErrorV1("applyReview.v1 received a non-markdown-artifact completed content");
  }
  const input = context.validatedInput as ApplyReviewActionInputV1;
  ...
  if (result.kind !== "ok") {
    throw new ApplyReviewPromotionErrorV1(
      `could not write artifact ${input.targetLocator.relativePath}: ${result.kind}${
...
export function createApplyReviewRowV1(): ProviderTaskActionRowV1 {
  return {
    kind: "provider",
    ...
    buildPrompt: (context: TaskActionExecutionContextV1): string =>
      (context.validatedInput as ApplyReviewActionInputV1).prompt,
    promoteCompletedContent: promoteApplyReviewContentV1,
  };
}
