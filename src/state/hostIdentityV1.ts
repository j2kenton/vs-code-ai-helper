...
export async function resolveDurableHostIdentityV1(rootDir: string): Promise<string> {
  ...
  try {
    await fs.promises.writeFile(filePath, JSON.stringify(record), { flag: "wx" });
    return record.hostId;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "EEXIST") {
      // Cannot persist a durable identity for some other reason (permissions,
      // missing volume, ...) — fail open to an ephemeral id rather than
      // blocking every admission call on a storage problem outside its scope.
  ...
  try {
    ...
    // Corrupt or unreadable — fall through to an ephemeral id below.
  }
  return `ephemeral-${randomUUID()}`;
}
