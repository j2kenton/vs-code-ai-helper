...
export function buildPreflightToolSessionPreambleV1(
  ...
  return [
    ...
    "  - `replaceFile` / `deleteFile` require an observation of kind `file`",
    ...
    "",
    "File content is carried as `contentBase64`: base64 of the COMPLETE new file",
    "bytes, not a diff or a fragment. Do not add a length or checksum field — the",
    "host derives those from the content itself.",
  ].join("\n");
}
