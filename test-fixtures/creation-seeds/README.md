...
| `legacy-v1-bare-seed.md` | Legacy bare V1 | The bare V1 seed (`# Task`, `## Task Description`, `## Draft with AI`, no typing gap or hint) emitted from the `draft.v1` cutover until the template gained the empty-description typing gap and the Draft with AI hint. | **Pinned.** Byte-identical to `resources/prompts/task-template.md` as committed at `3e467f7` (`sha256:2175edf8…`) — and, at that same revision, to `startNewTask.ts`'s inline fallback string too. Ships as `resources/prompts/creation-seed-legacy-v1-bare.md`. |
| `v1-canonical-seed.md` | V1 Canonical | Canonical seed emitted by `resources/prompts/task-template.md` and `startNewTask.ts` inline fallback. Contains `# Task`, `## Task Description` followed by a 15-blank-line typing gap, and `## Draft with AI` followed by the bracketed Draft with AI hint; UTF-8 without BOM, LF line endings (`\n`), and one final newline. Omits `## Open Questions`. | **Pinned.** Byte-identical to `resources/prompts/task-template.md` (current — the typing-gap/hint revision; `sha256:73a6b3b5…`). |
