...
8. If the review reports NO blockers but this task's work is not finished — look for a `<!-- progress: N/M -->` marker where N is less than M, or a verdict naming steps still to come — then this round's job is to **build the next steps**, continuing in the plan's own order from where the last round stopped. "No blockers" means nothing is wrong with what exists; it does not mean the work is done. Reporting "no changes needed" while steps remain unbuilt stalls the task, because this same review-then-implement cycle is what carries a large plan to completion a batch at a time. Implement as much of the remaining order as you can do well in one round — correctness and verification first, volume second — then state exactly which steps you completed and which are still outstanding.
