...
A checklist item whose line ENDS WITH the literal marker `<!-- ensemble:excluded -->` is already settled — closed WITHOUT doing the work (descoped, superseded, or a branch not taken) rather than closed by completing it. The marker only settles the item when it is appended to the end of that item's own line, matching production's own parsing rule (`trimEnd().endsWith(...)`); an item whose text merely mentions or quotes the marker string elsewhere in its sentence (for example, a not-yet-built step that describes adding the marker to OTHER items) does not carry it and remains outstanding. Treat a genuinely marked item exactly like a ticked (`- [x]`) item for readiness purposes: it is not outstanding work, and its absence from `## Verified Complete` above is expected, not a gap to flag.
